...
@Component({
  ...
  template: `
    <div class="min-h-screen bg-[#030f06] flex">
      <aside class="w-56 shrink-0 bg-[#060e07] border-r border-[#143c1a] flex flex-col">
        ...
        <nav class="flex-1 p-4 space-y-1">
          <a
            ...
            routerLinkActive="bg-[#0a2912] text-primary"
        ...
        <div class="p-4 border-t border-[#143c1a]">
          <button (click)="logout()" class="w-full flex items-center gap-3 px-3 py-2 rounded-lg text-gray-400 hover:text-red-400 hover:bg-red-950/20 transition text-sm">
            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      <main class="flex-1 overflow-auto" fadeIn>
        <router-outlet />
      </main>
    </div>
  `,
