    ...
    <section trackBehavior="hero_viewed" class="flex flex-col items-center justify-center text-white p-8 md:p-16">
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-8 w-full">
        @for (card of adaptiveCards(); track card.title) {
          <div trackBehavior="hero_card_viewed_{{ card.title }}"
               class="bg-surface border border-[#143c1a] rounded-2xl p-8 shadow-lg">
            ...
            <ul class="space-y-4 text-sm text-gray-300">
              @for (item of card.items; track item.title) {
                <li class="flex items-start">
                  ...
                  <span><strong class="text-white">{{ item.title }}</strong> {{ item.description }}</span>
                </li>
              }
            </ul>
          </div>
    ...
    return this.cards().map(baseCard => {
      ...
      if (isFrontend) {
        ...
        if (type === 'founder') {
          ...
          card.items[1] = { title: "Testing Culture:", description: "Enforcing TDD methodologies and rigorous API integration testing to guarantee enterprise platform stability." };
          ...
          card.items[1] = { title: "Robust Testing:", description: "Writing deterministic backend test suites using JUnit 5, Mockito, and Testcontainers for real DB integration." };
        }
      }
