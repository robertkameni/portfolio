  ...
  template: `
    <main class="min-h-screen bg-background text-white px-6 py-10 md:px-12 lg:px-16">
      <div class="max-w-5xl mx-auto">
        <a routerLink="/projects" class="inline-flex items-center gap-2 text-sm text-gray-400 hover:text-primary transition mb-8">
            ...
            @if (project.tags.length > 0) {
              <section class="flex flex-wrap gap-2">
                @for (tag of project.tags; track tag) {
                  <span class="rounded-full bg-[#07200f] px-3 py-1 text-xs text-primary border border-[#143c1a]">
                    {{ tag }}
                  </span>
                }
              </section>
...
export default class ProjectOverviewPage {
  ...
  projectResource = httpResource<Project>(() => this.slug() ? `/api/projects/${this.slug()}` : undefined);
  ...
  getErrorMessage(error: unknown): string {
    ...
    if (e?.status === 404) {
      return 'The project slug does not exist or the project is not published yet.';
    }
